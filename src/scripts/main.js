...
salesByWeek.forEach(cars => {
    infoContainer.innerHTML += `<h3>${cars.sales_agent.first_name} ${cars.sales_agent.last_name}</h3>`
    for (const carProperties of Object.entries(cars.vehicle)) {
        infoContainer.innerHTML += `<div>${carProperties[0]}: ${carProperties[1]}`
    }
    infoContainer.innerHTML += `<h5>Profit: $${cars.gross_profit}</h5>`
})
